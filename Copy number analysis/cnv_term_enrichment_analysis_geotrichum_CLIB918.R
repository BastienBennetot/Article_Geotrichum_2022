...
df<-read.csv2("ref_CLIB918_CNV_list_interet.csv")
ref="ref_CLIB918"
...
gene$chr<-as.numeric(str_remove_all(gene$Contig,pattern = "CCBN010|\\.1")) #Keep only numbers in scaffold column
...
write.table("enrichment_CNV_CLIB918.csv",x = test,row.names = F,sep = "\t")
