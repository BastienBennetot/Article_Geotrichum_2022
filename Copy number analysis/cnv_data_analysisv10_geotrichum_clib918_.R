...
temp = list.files(path = "data/ref_CLIB918",full.names = T,pattern="*CNVs.p.value.txt")
...
df$strains<-str_remove_all(df$source,pattern = "data/ref_CLIB918/|_q1_sorted_score_pos_markdup.bam_CNVs.p.value.txt")
...
#borders<-read.table(file = "563_pacbio_tgs_gapfiller.fasta.fai",sep = "\t",header =F,col.names = c("chr","length","cumul","NA1","NA2"))
  borders<-read.table(file = "CLIB918.fasta.fai",sep = "\t",header =F,col.names = c("chr","length","cumul","NA1","NA2"))
borders$chr<-as.numeric(str_remove_all(borders$chr,pattern = "CCBN01000|\\.1"))
...
## categorizing cnv --------------------------------------------------------
#CNV that differ between geoC and GeoA
temp<-temp%>%
...
temp<-temp%>%
  ...
  mutate(cheese_diff_cnv=case_when((median_cnv_variation[population%in%"Cheese_1"] %nin% median_cnv_variation[population %in% c("Cheese_2","Cheese_3")])~1,
        ...
        (median_cnv_variation[population%in%"Cheese_3"] %nin% median_cnv_variation[population %in% c("Cheese_1","Cheese_2")])~1,TRUE~0
))
...
all_cnv_interet$scaffold_name<- "CCBN0100000"
