...
df=read.table("outlier_genomic_scan.csv",sep = ";",header = T)
library(dplyr)
library(tidyr)
library(stringr)
library(stats)
...
test=df%>%group_by(pos.scaffold,begin)%>%filter(n()>1)
test=test%>%group_by(pos.scaffold,begin)%>%filter(any(analysis%in%"pi"))
df=test
...
write.csv(genomic_scan_gene,file = "genomic_scan_annotated_merging_dxy_pi.csv",row.names = F)
...
unique(temp$GeneID)%>%length()
unique(genomic_scan_gene$begin)%>%length()
